...
## 担当箇所
画像処理のグレースケール化を担当。
watchdog班から送られてくるパスを受け取り、それを元にopencvでグレースケールの状態で読み込み→ファイルに出力

##  チーム作業のログ

<pre>
* commit 6c62232c157de56c7e7ea6f3e540416b0a41caf7 (HEAD -> main, origin/main, origin/HEAD)
...
## 感想
ブランチを切り離すタイミングと、コミュニケーションの重要性がよく分かった。ブランチを切り離した後にファイルやフォルダの名前の変更、追加削除
を行う場合、それをチーム内で共有しておかないと、コンフリクトが発生するため、これを防ぐためのルールをあらかじめ決めておく必要があると感じた。
誰が今何の作業をしているかをチームリーダーだけでなく、各個人も把握しておくと良いことも分かった。

  Author: x20064Naruse <68636467+x20064Naruse@users.noreply.github.com>タイミング
  Author: x20064Naruse <68636467+x20064Naruse@users.noreply.github.com>
